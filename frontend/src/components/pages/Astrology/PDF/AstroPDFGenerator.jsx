...
const AstroPDFGenerator = React.forwardRef(({ allData, p_name }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    let formatedDOB;
    let dob = allData[0].DOB; // or just personalData.DOB
  if (dob && dob.includes("-")) {
    let parts = dob.split("-");
    if (parts.length === 3) {
      formatedDOB = `${parts[2]}-${parts[1]}-${parts[0]}`;
      console.log(formatedDOB); // Output: 14-07-2004
    } else {
      console.error("DOB format invalid");
    }
  } else {
    console.error("DOB is missing or in wrong format");
  }
  ...
  return (
    <div ref={pdfRef}>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-indigo-600 text-white flex flex-col justify-center items-center"
          ...
          <p className="text-3xl mt-4">{p_name}'s</p>
          <h1 className="text-6xl font-extrabold">Astrology Report</h1>
          <p className="text-3xl mt-4">Date of Birth : {formatedDOB}</p>
          <h1 className="text-xl font-extrabold">Report Generated By The Astro Pulse</h1>
          <h1 className="text-xl">Project Made By <strong>Utsav Lankapati</strong></h1>
        </div>
