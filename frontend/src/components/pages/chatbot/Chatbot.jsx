  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_ASTRO_API_URL}/chat?question=${encodeURIComponent(inputValue)}`,
        {
          method: "POST",
          headers: {
      ...
      const botReply =
        typeof data === "string" ? data : data.reply || "No reply received";
      ...
      {isOpen && (
        <div className="mt-4 w-[350px] h-[500px] flex flex-col bg-white/15 backdrop-blur-md border border-white/20 rounded-xl shadow-lg overflow-hidden">
          <div className="bg-[#451B6D] text-white flex items-center justify-between px-4 py-2">
            ...
            <button onClick={toggleChat} aria-label="Close chat window">
              {" "}
              {/* Added aria-label */}
            ...
            {messages.map((message, index) => {
              ...
              return (
                ...
                >
                  ...
                  {isLong && (
                    <div
                      className={`text-xs mt-1 text-right italic ${
                        message.sender === "user"
                          ? "text-gray-100"
                          : "text-gray-700" // Adjusted text color for bot messages
                      }`}
                    >
